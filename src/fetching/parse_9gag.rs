...
pub fn parse_9gag_posts(text: &str) -> Vec<Post> {
...
fn parse_9gag_post(text: &str) -> Vec<Post> {
    ...
    // TODO: change the logic to work with the new regex which parses the PostImage
    let reg = Regex::new(
        r#""id":"(.+?)","url":"(.+?)","title":"(.+?)","type":"(.+?)".+?"upVoteCount":(\d+),"hasLongPostCover":(\d),"images":\{("(.+?)":\{"width":(\d+),"height":(\d+),"url":(.+?)","webpUrl":"(.+?)"\},?)+"#
  ).expect("Couldn't parse the post data using regex");

    // parse the overall posts of the data
    for group in reg.captures_iter(text) {
        posts.push(Post {
            ...
            vote_count: group[5].parse::<u32>().expect(
                "Couldn't parse votes count to a number",
            ),
            is_long: false,
