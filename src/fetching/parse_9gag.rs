...
fn parse_9gag_post(text: &str) {
  ...
  for group in reg.captures_iter(text) {
    println!("id: {}", &group[1]);
    println!("url: {}", &group[2]);
    println!("title: {}", &group[3]);
    println!("type: {}", &group[4]);
    println!("vote count: {}", &group[5]);
    println!("is long: {}", &group[6]);
    println!("images: {}", &group[7]);
  }
}

#[cfg(test)]
mod tests {
  use super::parse_9gag_post;
